...
\definecolor{navyblue}{RGB}{28, 43, 74}
\definecolor{teal}{RGB}{2, 128, 144}
\definecolor{lightteal}{RGB}{2, 195, 154}
\definecolor{orange}{RGB}{232, 122, 43}
...
\setbeamercolor{subtitle}{fg=lightteal}
...
\subtitle{A Multi-Stage Pipeline for Reordering\\Dependency Structure Matrices}
...
\begin{frame}{Motivation}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{The challenge:}
      \begin{itemize}
        \item Modern software \& engineering projects involve \highlight{hundreds of interdependent tasks}
        \item Require coordination of large, multidisciplinary teams
        \item Traditional tools (Gantt, CPM) model sequential \& parallel flows
        \item \alertword{fail for feedback dependencies}, common in complex product development
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Goal}
        Find a \textbf{task ordering} that minimises conflicting information flows to enable faster,
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item Diagonal entries carry no inter-task meaning
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{General case}
        Achieve \highlight{Block Lower Triangular Form (BLTF)}: group cycle-participants into diagonal blocks and minimise feedback distance.
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Our 6-Stage Pipeline}
  ...
  \begin{tikzpicture}[
    ...
    \node[below=0.35cm of csc,  font=\tiny, color=slategray] {$O(n+\text{nnz})$};
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Why CSC?}
        Column $j$ lists out-neighbours of vertex $j$,\\
        giving $O(\deg^+(j))$ access — exactly the\\
        pattern required for DFS-based SCC algorithms.
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{SCC Decomposition}
  Both algorithms implemented as \textbf{iterative DFS} over CSC, using an explicit stack (no recursion).
...
\begin{frame}{Tearing \& Banding}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Tearing (intra-block FBM reduction)}
        \begin{itemize}
          \item Approximates the \textbf{minimum feedback arc set} within each non-trivial SCC
          \item Uses \textbf{Algorithm~GR} (Eades et al., 1993): greedily removes sinks, then sources, then the vertex with maximum $\delta = d^+ - d^-$
          \item Backward edges in the returned ordering are torn
          \item $O(k^2 + m)$ per SCC (linear scan variant)
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Banding (TFBD reduction)}
        \begin{itemize}
          \item \textbf{Reverse Cuthill-McKee (RCM)}: concentrates nonzeros near diagonal
          \item Pseudoperipheral start via George-Liu BFS
          \item \textbf{Adjacent-swap refinement}: accepts swaps that decrease TFBD without increasing FBM; up to 20 passes
          \item Best of torn + RCM orderings is kept
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
  ...
  \vspace{0.4em}
  \begin{alertblock}{Benchmark data}
    10 sparse matrices from the project repository, $n = 11$ to $27{,}770$, nnz $= 4$ to $352{,}807$.
    Compiled with \texttt{-O3}, Apple LLVM 17, Apple M2 / 8\,GB RAM.
...
\begin{frame}{Qualitative Results — Matrix Spy Plots}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \textbf{California} ($n = 9{,}664$)\\
      Near-perfect partitioning: almost no residual FBM, no large SCC blocks visible.

      \vspace{0.8em}
      \textbf{wb-cs-stanford} ($n = 9{,}914$)\\
      One large persistent SCC block retains significant FBM far from diagonal — explains high residual TFBD.
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{Tina\_AskCal} ($n = 1{,}023$)\\
      Clean block structure with well-separated components after reordering.

      \vspace{0.8em}
      \textbf{GD99\_c} ($n = 62$)\\
      Illustrates the SCC structure clearly: many trivial singleton and two-node SCCs, plus two larger coupled blocks.
    \end{column}
  \end{columns}
  \vspace{0.5em}
  \begin{center}
    \textit{(See appendix figures for full spy plot comparisons before and after partitioning.)}
  \end{center}
\end{frame}
...
\begin{frame}{Acknowledgements \& Team Contributions}
  \begin{block}{AI Disclosure}
    Claude (Anthropic) and ChatGPT (OpenAI) were used to aid understanding of algorithmic
    concepts and literature (SCC decomposition, tearing, banding). No AI tool was used
    to generate code or write report text.
  \end{block}
  ...
  \begin{block}{Team Member Contributions}
    \begin{tabular}{ll}
      ...
      \midrule
      Simon Kraft   & [e.g., implemented Tarjan's algorithm; wrote Sections 2.1--2.3] \\
      Joshua Payne  & [e.g., implemented Kosaraju-Sharir; wrote Sections 2.4--2.5] \\
      El Sall       & [e.g., implemented tearing \& banding; wrote Sections 3 \& 4] \\
      \bottomrule
    \end{tabular}
  \end{block}
