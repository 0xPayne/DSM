  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Tearing (reduces FBM within SCCs)}
        \begin{itemize}
          \item Reorder tasks within each SCC to reduce feed-back marks
          \item Uses \textbf{Algorithm~GR}: greedily removes sinks, sources, then the vertex with the largest degree difference
          \item Backward edges in the resulting ordering are the ``torn'' feedback arcs
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Banding (reduces TFBD within SCCs)}
        \begin{itemize}
          \item Push remaining feed-back marks closer to the diagonal
...
\begin{frame}{Experimental Setup}
  ...
  \begin{block}{Hardware \& methodology}
    \begin{itemize}
      \item Apple M2 / 8\,GB RAM, compiled with \texttt{-O3}, Apple LLVM 17
      \item Each algorithm: 2 warm-up + 5 timed runs
      \item Metrics: FBM count, TFBD, and runtime ($\mu$s)
    \end{itemize}
  \end{block}
\end{frame}
